...
## Evaluate across a surface × model matrix, not just once
...
- Run the same prompt suite (Steps 1–6 above) against **every surface** the skill is exposed on.
- Within each surface, run it against **every model tier** customers might realistically be routed to.
- Track scores **per cell of the matrix**, not just as one overall aggregate — a regression that gets averaged away by strong scores elsewhere is still a regression a real customer will hit on that specific surface/model pairing.

This has a real, recurring cost: every additional surface or model multiplies the runtime environments to stand up, the traces to capture, and the historical results to store and compare over time. It's not a one-time expense — a fix validated on one cell doesn't guarantee the others stayed correct, so the whole matrix needs periodic re-validation, not just the cell that changed.
...
- **The surface × model matrix keeps growing.** New model versions and new surfaces ship on an ongoing basis, and each addition expands the matrix that needs re-validation — a skill that passed evals yesterday isn't guaranteed to still pass once a new model becomes the default for a surface, or a dependent tool is updated underneath it. *Address it by treating new model and tool releases as an eval trigger in their own right* — re-run the existing prompt suite against the new cell before it reaches customers, and treat any resulting gap (say, a smaller model skipping a step a larger one handled fine) as a prompt-tuning fix to make, not an acceptable inconsistency to leave in place.

For what running this continuously actually looks like — shared fixtures, combined tool+judge verdicts, run tracing, durable artifact storage, and a CI pipeline that gates changes pre-merge — see [Operationalizing Skill Evals](operationalizing-skill-evals.html).

## Mental model

> Evaluating a skill means testing three layers at once — *did it use the right tools, in the right order* (mechanical correctness), *did it produce a good result* (quality, judged along explicit dimensions), and *can this run unattended, safely, repeatably* (isolated identity, full capture). Running that evaluation *continuously*, across every surface and model a skill is actually exposed on, adds a fourth layer — *can failures be triaged and routed faster than they accumulate, across a matrix that keeps growing* — and that layer is solved with the same instinct as the others: capture everything, classify automatically wherever the signal allows it, and reserve human judgment for the cases that genuinely need it.
