...
![A client talking to a federating gateway, which routes to a shared front-door tool plus three independent upstream MCP servers](../assets/diagrams/mcp-gateway-federation.png)
...
**Renaming and namespacing prevent most collisions before they happen.** Two independent upstreams naming a tool the same thing is the default outcome of independent teams solving similar problems, not an edge case. Giving every upstream a default namespacing prefix — or an explicit rename for the specific tools that need one — means most collisions never occur in the first place, rather than needing to be resolved after the fact. This also lets the gateway present a single, consistent naming convention to clients even when the underlying teams never agreed on one.

![Upstream tools a, b, c, secret passing through include, exclude, rename, and prefix stages to become the final exposed tool set](../assets/diagrams/fed-filter-pipeline.png)

### Detecting and containing the collisions that get through anyway

Naming discipline reduces collisions; it doesn't guarantee they can't happen, especially as the number of federated upstreams grows and nobody has full visibility into every other team's naming choices. Treat detection and containment as independent layers on top of naming discipline, not a replacement for it:

**Fail loud, never fail silent.** When merging tools from every upstream into one exposed list, track which names are already claimed. If a later upstream tries to claim a name that's taken, don't let it silently shadow the existing one — log a hard error and drop the duplicate instead. Silently picking a winner is worse than dropping both: a tool that quietly routes to the wrong backend some fraction of the time is far harder to debug than one that's simply missing and throws an obvious, immediate error.

![Backend A and backend B both trying to expose a tool named generate, the registry detecting the collision, logging an error, and dropping the duplicate](../assets/diagrams/fed-collision.png)

**Check the union, not just pairwise, and check it before it ships.** The check needs to run over the combined exposed name-space across *every* upstream, not just newly-added ones against existing ones. Doing this as an automated step whenever a backend's configuration changes — rather than only when discovery runs live — catches a collision in review instead of in production.

**Treat any wildcard-including backend as the highest-risk case.** A backend configured to expose "everything" it has can't be checked against a static name list in advance, since its tool names aren't known until it's actually queried. Keep this kind of backend as narrowly scoped as possible — ideally limited to only the tools genuinely unique to it — rather than letting it blindly re-expose tools another backend already owns under a different name. This risk is easy to underestimate precisely because it's invisible until the wildcard backend happens to add a tool that collides.

**Authorization scoping is itself a collision-prevention lever, not just an access-control one.** If per-backend authorization is evaluated *before* the step that merges results from every backend into one flat list, an unauthorized duplicate gets rejected before it can ever reach the collision check at all. Narrowing which tools a given backend is *allowed* to expose — even below what it's technically capable of returning — is a second, independent way of guaranteeing a name never collides, layered on top of naming discipline rather than substituting for it.
...
![A shared HTTP client with one mutated Authorization header letting two concurrent requests race, versus an ephemeral client per call keeping each request's token isolated](../assets/diagrams/fed-ephemeral-bug.png)
